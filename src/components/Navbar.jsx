...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="navbar navbar1" style={{ color: "white" }}>
        <Link to="/">
          {/* <img src={logo} alt="logo" className="w-full" /> */}
          <h1 className="w-full h1style">Mern Blog</h1>
        </Link>
        ...
        {/* <div
          ...
          > */}
        <div
          className={`search1 absolute w-full left-0 top-full mt-0.5 border-b py-4 px-[5vw] md:border-0 md:relative md:inset-0 md:p-0 md:w-auto md:block ${
            clickEvent ? "show" : "hide"
          }`}
        >
          <input
            className=" search1 w-full md:w-auto p-3 pl-6 pr-[12%] md:pr-6 rounded-full md:pl-12 bg-red-50"
            type="text"
            placeholder="Search"
          ...
          {accessToken ? (
            <>
              <Link to="/dashboard/notifications">
                <button className="notify relative w-12 h-12 rounded-full fa-solid bg-red-100/10 hover:bg-red-200">
                  <i className="fa-regular fa-bell text-2xl block fa-solid"></i>
                  {new_notification_available ? (
                    <span className="bg-red-500 w-3 h-3 fa-solid rounded-full absolute z-10 top-2 right-2"></span>
                  ) : (
                    ""
                  )}
                </button>
